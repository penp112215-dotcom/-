...
interface UsStockDisplay extends UsStock {
  cardCls: string
  badgeCls: string
  badgeText: string
  changeCls: string
...
function assertPortfolioResponse(res: PortfolioApiResponse) {
...
function normalizeUsStock(raw: Record<string, unknown>): UsStock {
  return {
...
function normalizeCryptoPerp(raw: Record<string, unknown>): CryptoPerp {
  return {
...
function extractPortfolioPayload(res: PortfolioApiResponse | null | undefined) {
  const root = 
      res && typeof res === 'object' && (res as any).data && typeof (res as any).data === 'object'
        ? ((res as any).data as PortfolioApiResponse)
        : (res || {})

  const stocksRaw = root.us_stocks || root.usStocks
  const usStocks = Array.isArray(stocksRaw)
    ? stocksRaw.map((item) => normalizeUsStock(item as Record<string, unknown>))
    : []

  const perpRaw = root.crypto_perp || root.cryptoPerp
  const cryptoPerp =
    perpRaw && typeof perpRaw === 'object'
      ? normalizeCryptoPerp(perpRaw as Record<string, unknown>)
      : normalizeCryptoPerp({})
...
function formatPrice(price: number | null, prefix: string): string {
  if (price == null) return prefix + '--'
  return prefix + price
}

function toUsStockDisplay(item: UsStock): UsStockDisplay {
...
function toCryptoDisplay(item: CryptoPerp): CryptoPerpDisplay {
...
function mapUsStocks(items: UsStock[]): UsStockDisplay[] {
  if (!Array.isArray(items)) return []
  return items.map(toUsStockDisplay)
}

// 仅在网络完全失败时使用的前端占位
const placeholderUsStocks: UsStock[] = [
  {
    symbol: 'AAPL',
    cn_name: '苹果',
    name: 'Apple Inc.',
...
const placeholderCryptoPerp: CryptoPerp = {
  symbol: 'BTCUSDT',
  price: 43250.5,
  change_pct_24h: 2.34,
  high_24h: 44580.0,
...
Page({
  data: {
    usStocks: [] as UsStockDisplay[],
    cryptoPerp: {} as CryptoPerpDisplay,
    displayUpdatedAt: '--:--:--',
    loading: true,
    error: '',
  ...
  applyPortfolioData(usStocks: UsStock[], cryptoPerp: CryptoPerp, updatedAt: string) {
    this.setData({
  ...
  fetch(cb?: () => void) {
    ...
    request<PortfolioApiResponse>(API_PATH.PORTFOLIO, { timeout: 180000 })
      .then((res) => {
